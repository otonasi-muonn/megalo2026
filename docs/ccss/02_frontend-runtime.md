...
- 開発時は同一アプリシェルを維持し、起動モードで表示UIを切り替える。
- `vite` モード: 既存React UI（ホーム/ダッシュボード/ステージ作成）を表示。
- `ccss` モード: ルート（`/`）をCCSSランタイム表示へ切り替え、監査画面（`/ccss-audit`）と併用する。
- 切替は Vite の実行モード（`vite --mode ccss`）で行い、必要に応じて `VITE_FRONTEND_UI_MODE` で明示上書きできる。
